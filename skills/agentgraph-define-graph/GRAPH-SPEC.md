...
model: cheap                    # optional — host maps to its cheapest available model
...
| `model` | no | the `agent` type's own default | Overrides the model this node's subagent runs on. Bookkeeping nodes set `cheap`; the host maps `cheap` (and the legacy alias `haiku`) to its cheapest available model — do not halt. Use for mechanical nodes (e.g. a connectivity check). Not used on `type: map`/`subgraph` nodes whose `ref`/`ref_from` recurses into another graph — the model is set per-node inside the referenced graph.md instead. |
...
- **Dispatch `model` explicitly.** Bookkeeping / mechanical nodes set `model: cheap`. The
  host maps `cheap` (and the legacy alias `haiku`) to its cheapest available model. An
  omitted `model` inherits the (usually expensive) session default.
...
- **Preferred structural index (CBM).** Prefer `codebase-memory-mcp` (`search_graph`,
  `detect_changes`, `index_status`, `list_projects`) when it is connected. `04_load_tasks`
  (and the planner) write `CBM: connected` or `CBM: missing` into `agent_works/INDEX.md`.
  Missing is a **warning**, not a stop — continue with targeted file reads. Do not grep
  `Library/`, `PackageCache/`, `Temp/`, or `ThirdParty/` unless the task names that path.
  After a node writes source, `detect_changes` (or a re-read) before trusting the index
  for those files.
