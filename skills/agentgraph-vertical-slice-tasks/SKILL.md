...
format — tasks stay in `agent_works/plans/{slug}.tasks.json`, fanned out by the `feature-kickoff`
graph's task-execution nodes; there's no ticket-per-file output, no tracker publish step, and no
interactive breakdown quiz (the `tech-plan-reviewer` approval gate is the substitute).
