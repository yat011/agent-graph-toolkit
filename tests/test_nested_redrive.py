"""Nested feature-kickoff redrive must resume the child standard-phase thread.
...
def _implement_output(run_dir: Path) -> Path:
    return run_dir / "05_run_phases" / "item-1" / "02_implement_requirements" / "attempt-1" / "output.md"
...
def test_sqlite_new_process_nested_redrive_resumes_child_implement(monkeypatch, tmp_path):
    ...
    assert resumed[INTERRUPT_PARENT_NODE_KEY] == RUN_ONE_PHASE_NODE
...
def test_sqlite_new_process_retries_exhausted_redrive_runs_implement_again(
    ...
    assert payload[INTERRUPT_PARENT_NODE_KEY] == RUN_ONE_PHASE_NODE
    item1_impl = run_dir / "05_run_phases" / "item-1" / "02_implement_requirements"
